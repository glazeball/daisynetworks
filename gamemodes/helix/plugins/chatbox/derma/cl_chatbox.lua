...
-- gamemode/core/libs/cl_markup/MarkupObject:draw() override
-- NOTE: alpha is some sort of override, which is ALWAYS 255 afaik
local function PaintMarkupOverride(text, font, x, y, color, alignX, alignY, alpha, block)

	-- afaik this is always a string wich breaks SetTextColor silently
	-- this code (and cl_markup) is a nightmare.
	local textAlpha = tonumber(block.colour["a"]) or 255

	if (ix.option.Get("chatOutline", false)) then
		-- outlined background for even more visibility
		draw.SimpleTextOutlined(text, font, x, y, ColorAlpha(color, textAlpha), alignX, alignY, 1, Color(0, 0, 0, textAlpha))
	else
		-- background for easier reading
		surface.SetTextPos(x + SScaleMin(1 / 3), y + SScaleMin(1 / 3))
		...
		surface.SetFont(font)
		surface.SetTextColor(color.r, color.g, color.b, textAlpha)
		surface.DrawText(text)
	end
end
...
function PANEL:SetMarkup(text)
	...
	if !parentParent then return end
	if !parentParent.ScrollToBottom then return end
...
function PANEL:SizeToContents()
	...
	self:SetSize(width + SScaleMin(12 / 3), height + SScaleMin(6 / 3))
end
...
function PANEL:Init()
	...
	self.buttons.Paint = function(_, w, h)
		surface.SetDrawColor(Color(34, 32, 64, 100))
...
function PANEL:Init()
	-- smaller top margin to help blend tab button/history panel transition
	self:DockMargin(SScaleMin(4 / 3), SScaleMin(2 / 3), SScaleMin(4 / 3), SScaleMin(4 / 3))
...
function PANEL:PerformLayout(width, height)
	...
	local bScroll = !ix.gui.chat:GetActive() or bar.Scroll == bar.CanvasSize
...
function PANEL:AddLine(elements, bShouldScroll)
	...
	for _, v in ipairs(elements) do
		...
		elseif (istable(v) and v.r and v.g and v.b) then
			if (v.a) then
				buffer[#buffer + 1] = string.format("<color=%d,%d,%d,%d>", v.r, v.g, v.b, v.a)
			else
		...
		elseif (type(v) == "Player") then
			...
			buffer[#buffer + 1] = tostring(v):gsub("<", "&lt;"):gsub(">", "&gt;"):gsub("%b**", function(value)
				...
				if (inner:find("%S")) then
					if CHAT_CLASS.font == nil then CHAT_CLASS.font = "ixChatFont" end 
						
					if CHAT_CLASS.font == "ixChatFont" then 
	...
	panel:Dock(TOP)
	panel:DockMargin(SScaleMin(10 / 3), 0, SScaleMin(10 / 3), 0)
...
function PANEL:SetFont(font)
	...
	self:SetTall(height + SScaleMin(8 / 3))
end
...
function PANEL:Init()
	...
	self.name:Dock(TOP)
	self.name:DockMargin(SScaleMin(4 / 3), SScaleMin(4 / 3), 0, 0)
	...
	self.description:DockMargin(SScaleMin(4 / 3), SScaleMin(4 / 3), 0, SScaleMin(4 / 3))
...
function PANEL:Init()
	...
	entryPanel:DockMargin(SScaleMin(4 / 3), 0, SScaleMin(4 / 3), SScaleMin(4 / 3))
	...
	-- top margin is 3 to account for tab 1px border
	self.autocomplete:DockMargin(SScaleMin(4 / 3), SScaleMin(3 / 3), SScaleMin(4 / 3), SScaleMin(4 / 3))
