...
function GM:LoadFonts(font, genericFont)
	...
	surface.CreateFont("ixChatYellFont", {
		font = font,
		size = math.max(ScreenScale(9), 19) * ix.option.Get("chatFontScale", 1),
		extended = true,
		weight = 800,
		antialias = true
	})

	surface.CreateFont("ixChatYellFontItalic", {
		font = font,
		size = math.max(ScreenScale(9), 19) * ix.option.Get("chatFontScale", 1),
		extended = true,
		weight = 800,
...
function GM:HUDPaintBackground()
	...
	if (IsValid(weapon) and hook.Run("CanDrawAmmoHUD", weapon) != false and weapon.DrawAmmo != false) then
		...
		if (weapon:GetClass() != "weapon_slam" and clip > 0 or count > 0) then
			...
			ix.util.DrawText(
				(clip == -1 or clipMax == -1) and count or clip.."/"..count
				, x + scale64, y + scale32, nil, 1, 1, "SkillsDesc")
		end
	end
...
net.Receive("ixConfirmationRequest", function()
	local time = net.ReadUInt(32)
	local title, subTitle = net.ReadString(), net.ReadString()
	...
	Derma_Query(subTitle, title, L("yes"), function()
